...
# ESTRUCTURA --------------------------------------------------------------
# Script que extrae peso_actual a partir de evoluciones
...
# Ejecutar proceso de extracción de valor/es de peso_actual
...
path_archivo_peso_actual <- paste0(path_exporte, "signo_vital_peso_actual.csv")

# Si el archivo ya existe, eliminarlo
if (file.exists(path_archivo_peso_actual)) {
  file.remove(path_archivo_peso_actual)
}
...
# Ejecutar proceso de extracción de valor/es de peso_actual
source("~/agise/proyectos/entidad_peso_talla/proceso_extraccion_peso_actual.R")
...
# id_signo_vital, para peso 2

peso_actual_via_evolucion <- evoluciones_con_valor_peso_actual %>% 
  rename(valor = peso_actual) %>% 
  mutate(descripcion_signo = "peso_actual",
         unidad = "kg",
         medida_nro = 1,
         id_signo_vital = 2) %>% 
...
rm(evoluciones_con_valor_peso_actual)
...
readr::write_delim(peso_actual_via_evolucion, path_archivo_peso_actual, delim = ";")

encolar_archivo_productivo(path_archivo_peso_actual)
enviar_archivo_productivo()

rm(path_exporte, path_archivo_peso_actual)
rm(peso_actual_via_evolucion)
